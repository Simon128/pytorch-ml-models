...
from .data_formats import BBAVectorOutput
...
class DecDecoder(object):
    ...
    def ctdet_decode(self, pr_decs: BBAVectorOutput):
        heat = pr_decs.heatmap
        wh = pr_decs.vector_map
        reg = pr_decs.offset_map
        cls_theta = pr_decs.orientation_map
        ...
        for b in range(batch):
            index = (scores[b] > self.conf_thresh).squeeze(-1)
            new_detections.append(detections[b][index])
        return new_detections

    def to_pts_and_scores(self, predictions, down_ratio):
        num_batches = len(predictions)
        pts0 = []
        scores0 = []
        cls0 = []
        for b in range(num_batches):
            cen_pt = torch.stack([predictions[b][..., 0], predictions[b][..., 1]], dim=-1)
            tt = torch.stack([predictions[b][..., 2], predictions[b][..., 3]], dim=-1)
            rr = torch.stack([predictions[b][..., 4], predictions[b][..., 5]], dim=-1)
            bb = torch.stack([predictions[b][..., 6], predictions[b][..., 7]], dim=-1)
            ll = torch.stack([predictions[b][..., 8], predictions[b][..., 9]], dim=-1)
            tl = tt + ll - cen_pt
            bl = bb + ll - cen_pt
            tr = tt + rr - cen_pt
            br = bb + rr - cen_pt
